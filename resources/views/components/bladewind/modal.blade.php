...
<span
    class="sm:w-1/6 sm:w-1/5 sm:w-1/4 sm:w-1/3 sm:w-2/5 sm:w-2/3 sm:w-11/12"></span>

<div class="bw-modal bw-{{ $name }}-modal fixed left-0 top-0 z-40 flex hidden h-full w-full bg-black/40 backdrop-blur-md"
    aria-backdrop-can-close="{{ $backdrop_can_close }}">
    <div class="sm:{{ $sizes[$size] }} bw-{{ $name }} m-auto w-full p-4">
        <div
            class="rounded-lg bg-white drop-shadow-2xl dark:border dark:border-slate-800 dark:bg-slate-900">
            <div class="flex">
                @if ($type !== '')
                    <div class="modal-icon grow-0 py-6 pl-6">
                ...
                <div class="modal-body grow p-6">
                    <h1
                        class="modal-title text-left text-lg text-gray-800 dark:text-slate-300">
                        {{ $title }}</h1>
                    <div
                        class="modal-text pt-2 text-left text-base leading-6 tracking-wide text-gray-600 dark:text-gray-400">
                        {{ $slot }}
                    </div>
                </div>
            </div>
            @if ($show_action_buttons)
                <div
                    class="modal-footer @if ($center_action_buttons || in_array($size, ['tiny', 'small', 'medium'])) text-center @else text-right @endif rounded-br-lg rounded-bl-lg bg-gray-100 py-3 px-6 dark:border-t dark:border-slate-800 dark:bg-slate-800/50">
                    <x-bladewind::button type="secondary"
                        size="{{ $button_size }}"
                        onclick="{!! $cancelAction !!}"
                        class="cancel {{ $cancelCss }}">
                        {{ $cancel_button_label }}</x-bladewind::button>

                    <x-bladewind::button size="{{ $button_size }}"
                        onclick="{!! $okAction !!}"
                        class="okay {{ $okCss }} ml-3">
                        {{ $ok_button_label }}</x-bladewind::button>
                </div>
            @endif
        </div>
    </div>
...
<script>
    dom_el('.bw-{{ $name }}-modal').addEventListener('click', function(
        e) {
        let backdrop_can_close = this.getAttribute(
            'aria-backdrop-can-close');
    ...
    if (dom_els('.bw-{{ $name }}-modal .modal-footer>button')) {
        dom_els('.bw-{{ $name }}-modal .modal-footer>button').forEach((
            el) => {
