...
@php
    ...
    if ($percentageLabelPosition !== $percentage_label_position) {
    ...
    if ($percentagePrefix !== $percentage_prefix) {
        $percentage_prefix = $percentagePrefix;
    }
    if ($percentageSuffix !== $percentage_suffix) {
        $percentage_suffix = $percentageSuffix;
    }
    if ($cssOverride !== $css_override) {
        $css_override = $cssOverride;
    }
    if ($barClass !== $bar_class) {
        $bar_class = $barClass;
    }
    //-----------------------------------------------------------------------
    ...
    if (!is_numeric($percentage_label_opacity * 1)) {
        $percentage_label_opacity = '100';
    }
@endphp
...
<div class="bw-progress-bar {{ $class }}">
    @if (
        ...
        <div
            class="{{ str_replace('top ', 'text-', $percentage_label_position) }} text-xs tracking-wider">
            {{ $percentage_prefix }} <span
                class="opacity-{{ $percentage_label_opacity }}">{{ $percentage }}%</span>
            {{ $percentage_suffix }}
        </div>
    @endif
    <div
        class="@if (!$transparent) bg-slate-200/70 dark:bg-slate-800 w-full @endif my-2 mt-1 rounded-full">
        <div style="width: {{ $percentage }}%"
            class="bg-{{ $color }}-{{ $color_weight[$shade] }} {{ $css_override }} bar-width animate__animated animate__fadeIn {{ $bar_class }} rounded-full py-1 text-center">
            @if ($show_percentage_label && $show_percentage_label_inline)
                <span
                    class="text-{{ $color }}-{{ $text_color_weight[$shade] }} px-2 text-xs">
                    {{ $percentage_prefix }} <span
                        class="opacity-{{ $percentage_label_opacity }}">{{ $percentage }}%</span>
                    {{ $percentage_suffix }}
                </span>
    ...
    @if (
        ...
        <div
            class="{{ str_replace('bottom ', 'text-', $percentage_label_position) }} text-xs tracking-wider">
            {{ $percentage_prefix }} <span
                class="opacity-{{ $percentage_label_opacity }}">{{ $percentage }}%</span>
            {{ $percentage_suffix }}
        </div>
