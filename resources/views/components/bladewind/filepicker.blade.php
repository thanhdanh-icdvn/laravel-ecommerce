...
<div
    class="bw-fp-{{ $name }} @if ($add_clearing) mb-3 @endif relative cursor-pointer rounded-md border-2 border-dashed border-gray-300 px-2 py-3 text-center dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 hover:dark:border-slate-600">
    <x-bladewind::icon name="document-text"
        class="absolute left-4 z-20 h-6 w-6 text-gray-300" />
    <svg xmlns="http://www.w3.org/2000/svg"
        class="absolute z-10 h-4 w-4 text-gray-300" fill="none"
        viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
    </svg>
    <x-bladewind::icon name="x-circle"
        class="clear absolute right-3 hidden h-8 w-8 cursor-pointer text-gray-600 hover:text-gray-800"
        type="solid" />
    <span class="zoom-out selection inline-block px-6 pl-10 text-gray-400/80">
        {{ $placeholder }}
        @if ($required)
    ...
    <div class="h-0 w-0 overflow-hidden">
        <input type="file" name="{{ $name }}"
            ...
            id="bw_{{ $name }}" accept="{{ $accepted_file_types }}" />
        <textarea
            class="b64-{{ $name }}@if ($required) required @endif"
            name="b64_{{ $name }}"></textarea>
    </div>
</div>

<script>
    dom_el('.bw-fp-{{ $name }}').addEventListener('drop', function(
    evt) {
    ...
    dom_el('.bw-fp-{{ $name }} .clear').addEventListener('click',
        function(e) {
            dom_el('.bw-fp-{{ $name }} .selection').innerHTML =
                '{{ $placeholder }}';
            dom_el('.bw-{{ $name }}').value = dom_el(
                '.b64-{{ $name }}').value = '';
            changeCss('.bw-fp-{{ $name }} .clear', 'hidden');
