...
@endphp
<div class="@if ($add_clearing) mb-3 @endif relative w-full">
    <textarea
    ...
    @if ($error_message !== '')
        <div
            class="{{ $name }}-inline-error hidden px-1 pt-2 text-xs text-red-500">
            {{ $error_message }}</div>
    @endif
    @if ($label !== '')
        <label for="{{ $name }}" class="form-label dark:peer-focus:pt-1"
            ...
            @if ($required == 'true')
                <span class="text-red-400/80"><svg
                        xmlns="http://www.w3.org/2000/svg"
                        class="-mt-1 inline-block h-2 w-2" viewBox="0 0 20 20"
                        fill="currentColor">
                        <path
