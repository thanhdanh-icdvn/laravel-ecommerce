...
@php
    ...
    if (!in_array($color, ['blue', 'red', 'yellow', 'green', 'blue', 'orange', 'purple', 'cyan', 'pink', 'black'])) {
        $color = 'blue';
    }
@endphp
...
<div {{ $attributes->merge(['class' => 'bw-bell relative inline-block']) }}>
    <svg xmlns="http://www.w3.org/2000/svg"
        class="w-{{ $sizing[$size]['bell'] }} h-{{ $sizing[$size]['bell'] }} @if ($invert) text-white @endif cursor-pointer"
    ...
    @if ($show_dot)
        <div
            class="w-{{ $sizing[$size]['dot'] }} h-{{ $sizing[$size]['dot'] }} {{ $coloring['bg'][$color] }} {{ $coloring['ring'][$color] }} @if ($animate_dot) animate-ping @endif absolute top-0 right-0 rounded-full ring-2 ring-offset-1">
        </div>
    @endif
</div>
