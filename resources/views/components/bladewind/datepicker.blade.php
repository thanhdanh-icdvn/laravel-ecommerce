...
@if ($type == 'single')
    <div x-data="app('{{ $default_date }}', '{{ strtoupper($format) }}')" x-init="[initDate(), getNoOfDays()]" x-cloak>
        <div class="relative w-full">
            <div
                class="pointer-events-none absolute inset-y-0 right-3 z-30 flex items-center pl-3">
            ...
            <input type="hidden" x-ref="date" :value="datepickerValue"
                value="{{ $default_date }}" />
            <x-bladewind::input {{-- class="bw-datepicker bw-input block w-full peer {{$name}}" --}} class="bw-datepicker"
                x-on:click="showDatepicker = !showDatepicker"
                x-model="datepickerValue"
                x-on:keydown.escape="showDatepicker = false" type="text"
                id="dtp-{{ $name }}" max_date="today"
                name="{{ $name }}" label="{{ $label }}"
                placeholder="{{ $placeholder }}"
                required="{{ $required }}" />

            <div class="absolute top-0 left-0 z-50 mt-12 bg-white p-4 shadow-md dark:bg-slate-600"
                style="width: 17rem" x-show.transition="showDatepicker"
                @click.away="showDatepicker = false">
                <div class="mb-2 flex items-center justify-between">
                    <div>
                        <span x-text="MONTH_NAMES[month]"
                            class="cursor-default text-lg font-bold text-gray-800 dark:text-gray-400"></span>
                        <span x-text="year"
                            class="ml-1 cursor-default text-lg font-normal text-gray-600"></span>
                    </div>
                    <div>
                        <button type="button"
                    ...
                    month = 12;
                            ...
                            <svg class="inline-flex h-6 w-6 text-gray-400"
                ...
                <div class="-mx-1 mb-3 flex flex-wrap">
                    <template x-for="(day, index) in DAYS"
                        ...
                        <div style="width: 14.26%" class="px-0.5">
                            <div x-text="day"
                                class="cursor-default text-center text-xs font-medium uppercase text-gray-800 dark:text-gray-400">
                            </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="-mx-1 flex flex-wrap">
                    ...
                    <template x-for="(date, dateIndex) in no_of_days"
                        ...
                        <div style="width: 14.28%" class="mb-1">
                            <div @click="getDateValue(date, '{{ $format }}')"
                                x-text="date"
                                class="cursor-pointer rounded-full text-center text-sm leading-8 transition duration-100 ease-in-out"
                                :class="{
                                    'bg-blue-200 dark:bg-slate-700': isToday(
                                        date) == true,
                                    'text-gray-600 dark:text-gray-100 hover:bg-blue-200 hover:dark:bg-slate-500': isToday(
                                        date) == false && isSelectedDate(
                                        date) == false,
...
@else
    <div class="grid grid-cols-2 gap-2">
        <div>
            <x-bladewind::datepicker name="{{ $date_from_name }}" type="single"
                placeholder="{{ $date_from_label }}"
                default_date="{{ $default_date_from ?? '' }}"
                required="{{ $required }}" label="{{ $date_from_label }}"
                format="{{ $format }}" />
        </div>
        <div>
            <x-bladewind::datepicker name="{{ $date_to_name }}" type="single"
                placeholder="{{ $date_to_label }}"
                default_date="{{ $default_date_to ?? '' }}"
                required="{{ $required }}" label="{{ $date_to_label }}"
                format="{{ $format }}" />
        </div>
    </div>
@endif
@once
    <script>
        ...
        const MONTH_SHORT_NAMES = [
            ...
            april.substr(0, 3), may.substr(0, 3), june.substr(0, 3), july
            .substr(0, 3),
            august.substr(0, 3), september.substr(0, 3), october.substr(0, 3),
            november.substr(0, 3), december.substr(0, 3)
        ];
        const DAYS = [
            sunday.substr(0, 3), monday.substr(0, 3), tuesday.substr(0, 3),
            wednesday.substr(0, 3),
