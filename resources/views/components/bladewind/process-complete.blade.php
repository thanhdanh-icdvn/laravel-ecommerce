...
@php
    ...
    if ($processCompletedAs !== $process_completed_as) {
        $process_completed_as = $processCompletedAs;
    }
    if ($buttonLabel !== $button_label) {
        $button_label = $buttonLabel;
    }
    if ($buttonAction !== $button_action) {
...
@endphp
<div
    class="{{ $name }} @if ($hide) hidden @endif {{ $class }} mt-6 text-center">
    @if ($process_completed_as === 'passed')
        <svg xmlns="http://www.w3.org/2000/svg"
            class="mx-auto block h-14 w-14 rounded-full bg-green-400 p-2 text-white"
    ...
    @else
        <svg xmlns="http://www.w3.org/2000/svg"
            class="mx-auto block h-14 w-14 rounded-full bg-red-400 p-2 text-white"
            fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @endif
    <div
        class="@if ($process_completed_as === 'passed') text-green-600 @else text-red-600 @endif my-3 text-sm">
        <span class="process-message">{{ $message }}</span>
        <div class="mt-8"></div>
        <x-bladewind::button type="secondary" size="tiny"
            onclick="{{ $button_action }}">{{ $button_label }}
        </x-bladewind::button>
    </div>
</div>
