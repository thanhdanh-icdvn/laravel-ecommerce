...
@endphp
<span class="hidden !border-red-400"></span>
<div
    class="bw-alert {{ $color[$shade][$type] }} {{ $color[$shade][$type . '_text'] }} {{ $class }} flex w-full rounded-md p-3">
    @if ($show_icon)
        <div class="pt-[1px]">
            <x-bladewind::modal-icon type="{{ $type }}"
                class="{{ $color[$shade][$type . '_icon_color'] ?? '' }} !h-6 !w-6" />
        </div>
    @endif
    ...
    @if ($show_close_icon)
        <div class="text-right">
            <x-bladewind::icon name="x-circle"
                class="ml-3 inline h-5 w-5 cursor-pointer pt-[1px] opacity-60 hover:opacity-100 dark:text-slate-100" />
        </div>
    @endif
</div>
