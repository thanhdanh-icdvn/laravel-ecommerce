...
@php
    ...
    if ($type == 'password' && $viewable) {
        $suffix =
            '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="show-pwd h-6 w-6 cursor-pointer" onclick="togglePassword(\'' .
            $name .
            '\', \'show\')">
<path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="hide-pwd hidden h-6 w-6 cursor-pointer" onclick="togglePassword(\'' .
            $name .
...
<div
    class="dv-{{ $name }} @if ($add_clearing) mb-3 @endif relative w-full">
    <input
        {{ $attributes->merge(['class' => "bw-input peer $is_required $name $placeholder_color"]) }}
        type="{{ $type }}" id="{{ $name }}"
        name="{{ $name }}" value="{{ $selected_value }}" autocomplete="off"
    ...
    @if (!empty($error_message))
        <div
            class="{{ $name }}-inline-error hidden p-1 text-xs text-red-500">
            {{ $error_message }}</div>
    @endif
    @if (!empty($label))
            ...
            @if ($required)
                <span class="text-red-300">
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="mt-[-2px] inline-block h-2 w-2"
    ...
    @endif
    @if (!empty($prefix))
        <div class="{{ $name }}-prefix prefix @if (!$transparent_prefix) bg-slate-100 border-2 border-slate-200 dark:border-slate-700 dark:bg-slate-900/50 dark:border-r-0 border-r-0 rounded-tl-md rounded-bl-md @endif absolute inset-y-0 left-0 z-20 inline-flex select-none items-center pl-3.5 pr-2 text-sm text-blue-900/50 dark:text-slate-400"
            data-transparency="{{ $transparent_prefix }}">
            @if ($prefix_is_icon)
                <x-bladewind::icon name='{!! $prefix !!}'
                    type="{{ $prefix_icon_type }}"
    ...
    @endif
    @if (!empty($suffix))
        <div class="{{ $name }}-suffix suffix @if (!$transparent_suffix) bg-slate-100 border-2 border-slate-200 border-l-0 dark:border-slate-700 dark:bg-slate-900/50 dark:border-l-0 rounded-tr-md rounded-br-md @endif absolute inset-y-0 right-0 z-20 inline-flex select-none items-center pl-3.5 pr-2 text-sm text-blue-900/50 dark:text-slate-400"
            data-transparency="{{ $transparent_prefix }}">
            @if ($suffix_is_icon)
                <x-bladewind::icon name='{!! $suffix !!}'
